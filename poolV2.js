...
async function init(port, ch = false, callfrom = false) {
  ...
  if (!ch) {
    ch = port.uniswapV2.chain; // xxxx note this is the default
    console.log("init ch=", ch);
  }
  maps.oldInit(ch);
  console.log("init ch=", ch);
...
async function sell(port, slippage = false, ratio = false) {
  try {
    ...
    let apy =
      Math.floor(
        ((1 + (pos.netVusd + pos.netSusd) / port.uniswapV2.pair.value) **
...
async function adjust(port) {
  try {
    ...
    if (parseFloat(newpos.netVusd) > port.snapshot.threshold) {
      ...
      let u = utils.checkWallet(newpos.positions, "USDC", port.uniswapV2.chain);
    ...
    if (
      ...
      port.snapshot.threshold * port.uniswapV2.pair.optRatio
    ) {
      let wc = utils.checkWallet(
        newpos.positions,
        "WETH",
...
async function getPositions(port, retries = 0) {
  try {
    ...
    //let q = await quote.oneFastQuote(chain.getAddress("WETH"),port.uniswapV2.chain);
    ...
    console.log("getPositionsMult.init 2nd call", port.uniswapV2.chain);
    await init(port, port.uniswapV2.chain, "port.uniswapV2.chain", web3.chain);
    npos = await univ2.getPositions(
      port.uniswapV2.wname,
      port.uniswapV2.walletAddress,
      q
    );
    //console.log(npos);
    pos = pos.concat(npos);
    ...
    let value = port.uniswapV2.pair.value + profit;
    let tradeThresh = value * port.uniswapV2.pair.threshRatio;
    ...
    let newpos = {
      ...
      threshRatio: port.uniswapV2.pair.threshRatio,
      vusd: vusd,
      vamt: vusd / q,
      susd: susd,
      quote: q,
...
async function calculate(port) {
  try {
    ...
    let apy =
      Math.floor(
        ((1 + (newpos.netVusd + newpos.netSusd) / port.uniswapV2.pair.value) **
